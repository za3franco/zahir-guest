...
export default function CampaignsSearch({ campaigns, searchPlaceholder, clearLabel, emptyLabel, colLabels }: Props) {
        ...
        <table className={styles.table}>
          <thead>
            <tr>
              ...
              <th>{colLabels.auditor}</th>
              <th>{colLabels.status}</th>
          ...
          <tbody>
            {!filtered.length ? (
              ...
              <tr key={c.id}>
                ...
                <td className={styles.muted}>{c.auditorName}</td>
                <td><span className="badge badge-sand">{c.statusLabel}</span></td>
        ...
        {!filtered.length ? (
          ...
          <a key={c.id} href={`/dashboard/campaigns/${c.id}`} className={styles.mobileCard}>
            <div className={styles.mobileCardTop}>
              ...
              <span className="badge badge-sand">{c.statusLabel}</span>
            </div>
            <div className={styles.mobileCardSub}>{c.propertyName}{c.propertyCity ? ` · ${c.propertyCity}` : ''}</div>
            <div className={styles.mobileCardMeta}>{c.auditorName} · {c.dateFormatted}</div>
          </a>
        ))}
      </div>
    </>
