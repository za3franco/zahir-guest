...
export async function POST(request: Request) {
  ...
  // 1. Create Supabase auth user with a magic link / invite
  const { data: authData, error: authError } = await supabaseAdmin.auth.admin.inviteUserByEmail(email, {
    ...
    redirectTo: `${process.env.NEXT_PUBLIC_APP_URL ?? 'https://www.zahirguest.com'}/auth/callback`,
  })
  ...
  // 3. Send branded invite email via Brevo
              ...
              <table cellpadding="0" cellspacing="0" style="margin:0 0 32px;">
                <tr>
                  <td style="background:#C8A45A;border-radius:8px;">
                    <a href="{{ .ConfirmationURL }}" style="display:inline-block;padding:14px 32px;color:#0D1B2A;font-size:14px;font-weight:600;text-decoration:none;letter-spacing:0.04em;">
                      Set my password →
                    </a>
                  </td>
                </tr>
...
<body style="margin:0;padding:0;background:#0D1B2A;font-family:'DM Sans',Arial,sans-serif;">
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#162236;border-radius:12px;overflow:hidden;border:1px solid #253549;">
          ...
          <tr>
            <td style="padding:32px 48px;">
              ...
              <p style="color:#9B9488;font-size:13px;line-height:1.6;margin:0;">
                Ce lien d'invitation expire dans 24 heures. Si vous n'attendiez pas cette invitation, vous pouvez ignorer cet email.
              </p>
            </td>
          </tr>
          <!-- Footer -->
  ...
  if (!brevoRes.ok) {
    ...
    // Don't fail the whole invite if email fails — user is created, just log the error
  }
